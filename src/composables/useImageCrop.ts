...
// Global map to store handlers by nodeId, accessed by imageCrop.ts
// Key is always String(nodeId) to ensure consistent lookup
export const cropWidgetChangeHandlers = new Map<
  string,
  CropWidgetChangeHandler
>()
...
export const useImageCrop = (nodeId: NodeId) => {
  ...
  const cropX = ref(0)
  const cropY = ref(0)
  const cropWidth = ref(512)
  const cropHeight = ref(512)
  ...
  const syncCropFromWidgets = () => {
    cropX.value = getWidgetValue('x')
    cropY.value = getWidgetValue('y')
    cropWidth.value = getWidgetValue('width') || 512
    cropHeight.value = getWidgetValue('height') || 512
  }

  const registerWidgetChangeHandler = () => {
    if (nodeId == null) return

    // Use String(id) to ensure consistent key type (node.id can be number or string)
    cropWidgetChangeHandlers.set(String(nodeId), (name, value) => {
      if (name === 'x') cropX.value = value
      else if (name === 'y') cropY.value = value
      else if (name === 'width') cropWidth.value = value
      else if (name === 'height') cropHeight.value = value
    })
  }

  const unregisterWidgetChangeHandler = () => {
    if (nodeId == null) return

    cropWidgetChangeHandlers.delete(String(nodeId))
  }
  ...
  const cropBoxStyle = computed(() => ({
    left: `${imageOffsetX.value + cropX.value * scaleFactor.value}px`,
    top: `${imageOffsetY.value + cropY.value * scaleFactor.value}px`,
